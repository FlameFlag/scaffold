...
and textual WASM bindings match what the current Rust and generated reference
sources would produce. The binary `.wasm` payload is generated before packaging
because its bytes can vary by build host. The `compile:wasm` command builds the
Rust crate in release mode with normalized path prefixes and runs `wasm-bindgen`
to emit
